#!/usr/bin/env python3
"""Unitree robot controller using DeepLabCut-style pose estimation."""
...
from cat_meme_locomotion.core.dlc_pose_extractor import DeepLabCutPoseExtractor, KeypointTrajectory


class UnitreeDLCController:
    """Controller using DeepLabCut-style pose estimation for motion control."""
    ...
    def load_unitree_robot(self):
        """Load Unitree robot."""
        print("🤖 Loading Unitree Go2 with DeepLabCut pose control...")
    ...
    def apply_dlc_motion(self, motion_data: Dict, keypoint_trajectories: Dict[str, KeypointTrajectory]):
        """Apply motion from DeepLabCut pose estimation."""
        self.motion_data = motion_data
        self.keypoint_trajectories = keypoint_trajectories
        
        print("\n🎮 Starting simulation with DeepLabCut pose control...")
            ...
            if traj.positions and len(traj.positions) > 0:
                ...
                if np.any(valid_indices):
                    ...
                    if len(valid_positions) > 0:
                        ...
                        # Normalize all positions (including low confidence ones)
        ...
        while self.scene.viewer.is_alive():
            ...
            # Use a cyclic motion pattern based on frame
            phase = (motion_frame / max_frames) * 2 * np.pi
            
            # Front legs - use front paw keypoints or generate motion
            if 'left_front_paw' in normalized_trajectories and motion_frame < len(normalized_trajectories['left_front_paw']):
                traj = normalized_trajectories['left_front_paw'][motion_frame]
            ...
            target_dof_pos[4] = 0.2 + self.motion_amplitude * 0.6 * (1 - y_norm)  # FL_thigh
            target_dof_pos[5] = -1.8 + self.motion_amplitude * 0.4 * y_norm  # FL_calf
            
            if 'right_front_paw' in normalized_trajectories and motion_frame < len(normalized_trajectories['right_front_paw']):
                traj = normalized_trajectories['right_front_paw'][motion_frame]
                y_norm = traj[1]
            else:
                # Generate synthetic motion (offset from left)
                y_norm = 0.5 + 0.3 * np.sin(phase + np.pi)
            
            target_dof_pos[1] = 0.2 + self.motion_amplitude * 0.6 * (1 - y_norm)  # FR_thigh
            target_dof_pos[2] = -1.8 + self.motion_amplitude * 0.4 * y_norm  # FR_calf
            
            # Rear legs - use rear paw keypoints
            if 'left_rear_paw' in normalized_trajectories and motion_frame < len(normalized_trajectories['left_rear_paw']):
                traj = normalized_trajectories['left_rear_paw'][motion_frame]
                y_norm = traj[1]
            else:
                # Generate synthetic motion (offset from front)
                y_norm = 0.5 + 0.3 * np.sin(phase + np.pi/2)
            
            target_dof_pos[10] = 0.4 + self.motion_amplitude * 0.8 * (1 - y_norm)  # RL_thigh
            target_dof_pos[11] = -1.8 + self.motion_amplitude * 0.4 * y_norm  # RL_calf
            
            if 'right_rear_paw' in normalized_trajectories and motion_frame < len(normalized_trajectories['right_rear_paw']):
                traj = normalized_trajectories['right_rear_paw'][motion_frame]
                y_norm = traj[1]
            else:
                # Generate synthetic motion
                y_norm = 0.5 + 0.3 * np.sin(phase - np.pi/2)
            
            target_dof_pos[7] = 0.4 + self.motion_amplitude * 0.8 * (1 - y_norm)  # RR_thigh
            target_dof_pos[8] = -1.8 + self.motion_amplitude * 0.4 * y_norm  # RR_calf
            ...
            if 'right_shoulder' in normalized_trajectories and motion_frame < len(normalized_trajectories['right_shoulder']):
                ...
                if np.sum(traj) > 0:
                    x_norm = traj[0]
                    target_dof_pos[0] = 0.2 * (0.5 - x_norm)  # FR_hip
...
def run_dlc_simulation():
    """Run simulation with DeepLabCut pose estimation."""
    import argparse
    from pathlib import Path
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description="Unitree robot with DeepLabCut pose estimation")
    parser.add_argument(
        "--gif", 
        type=str, 
        default="assets/gifs/chipi-chipi-chapa-chapa.gif",
        help="Path to the GIF or video file"
    )
    parser.add_argument(
        "--speed",
        type=float,
    ...
    print("🐱 Unitree Cat Motion with DeepLabCut Pose Estimation")
    print("=" * 50)
    ...
    print("\n📊 Extracting motion with DeepLabCut pose detection...")
    
    # For videos, limit frames
    if is_video:
        extractor = DeepLabCutPoseExtractor(str(input_path), max_frames=100, target_fps=10)
    else:
        extractor = DeepLabCutPoseExtractor(str(input_path))
    ...
    # Run simulation
    controller = UnitreeDLCController()
    ...
    controller.apply_dlc_motion(motion_data, extractor.keypoint_trajectories)
